...
- 套件管理器 `winget`（Windows 內建；用於安裝 Claude Code CLI）
...
```
claumon 0.16.0 — service installed and started (port 3131)
Dashboard: http://localhost:3131
claumon will start automatically on login.
```

此指令會在 Startup 資料夾建立啟動腳本，登入時以隱藏視窗自動執行：

```
C:\Users\<使用者>\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\claumon.vbs
```
...
| 指令 | 用途 |
| --- | --- |
| `claumon service status` | 查看服務是否運行 |
| `claumon service restart` | 修改設定或剛登入後重啟（立即重抓額度） |
| `claumon service uninstall` | 停止並移除背景服務 |
| `claumon update` | 檢查並更新至最新版（有服務時會自動重啟） |
...
### 7.1 `service install` 後啟動腳本為空（0 bytes）／服務未啟動

**現象**：`claumon.vbs` 大小為 0，服務沒跑。
**原因**：安裝當下 binary 被防毒/系統短暫鎖檔，導致寫入中斷。
**處理**：移除空檔後重跑安裝即可。
...
2. 若剛登入，重啟服務以立即重抓：`claumon service restart`
